...
class FuzzyGroupAggregator(Aggregator):
    ...
    def train(self):
        ...
        def update_membership():
            ...
            if self.measurement == "euclid":
                ...
                #     momentum=self.momentum,
                # )
            print("after: ", self.membership_mat[:2])
    ...
    def update_membership_euclid(self, membership_mat, cluster_learners, client_learners,
                                 global_fixed_m, fuzzy_m, momentum=None):
        # n_clusters = len(cluster_learners)
        # n_clients = len(client_learners)
        ...
        distances = self.model_l2_distance(client_models, cluster_models)  # shape: [n, k]
        distances = distances - self.trans * \
            distances.min(dim=1, keepdim=True).values
        # print("distances shape", distances.shape)
        print("distances ", distances[:1])
        membership_mat = self.distances_to_membership(
            distances, p, eps, clamp_max)

        if self.previous_membership_mat is not None:
            self.membership_mat = self.previous_membership_mat * \
                momentum + membership_mat * (1 - momentum)

        self.previous_membership_mat = membership_mat.clone()
