...
parser.add_argument(
    '--name', help='name of dataset to parse; default: sent140;', type=str, choices=DATASETS, default='sent140'
)
parser.add_argument(
    '--by_user', help='divide users into training and test set groups;', dest='user', action='store_true'
)
parser.add_argument(
    '--by_sample',
...
parser.add_argument('--frac', help='fraction in training set; default: 0.9;', type=float, default=0.9)
parser.add_argument('--seed', help='seed for random partitioning of test/train data', type=int, default=None)
...
if (args.user):
    ...
    for f in files:
        ...
        if include_hierarchy:
            user_files.extend(
                [(u, h, ns, f) for (u, h, ns) in zip(data['users'], data['hierarchies'], data['num_samples'])]
            )
        else:
            user_files.extend([(u, ns, f) for (u, ns) in zip(data['users'], data['num_samples'])])
