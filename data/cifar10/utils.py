...
def split_dataset_by_labels(dataset, n_classes, n_clients, n_clusters, alpha, frac, seed=1234):
    ...
    # print("all_labels after shuffle ",all_labels)
    ...
    # print("clusters_sizes  ",clusters_sizes)
    # print("clusters  ",clusters)

    for _, cluster in clusters.items():
        rng.shuffle(cluster)
    # print("clusters after shuffle",clusters)

    clients_counts = np.zeros(
        (n_clusters, n_clients), dtype=np.int64
    )                    # number of samples by client from each cluster
    ...
    for cluster_id in range(n_clusters):
        ...
        # print("clients_counts  ",cluster_id,"  ", clients_counts[cluster_id])

    clients_counts = np.cumsum(clients_counts, axis=1)
    # print("clients_counts  ", clients_counts)

    clients_indices = [[] for _ in range(n_clients)]
    for cluster_id in range(n_clusters):
        cluster_split = split_list_by_indices(clusters[cluster_id], clients_counts[cluster_id])
        # print(f"split_list_by_indices cluster{cluster_id} split  \n",cluster_split)

        for client_id, indices in enumerate(cluster_split):
            clients_indices[client_id] += indices

        # print("split_dataset_by_labels clients_indices \n",clients_indices)
