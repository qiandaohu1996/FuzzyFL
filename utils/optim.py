...
def get_optimizer(optimizer_name, model, lr_initial, freq=10, mu=0.):
    ...
    if optimizer_name == "adam":
        return optim.Adam(
            [param for param in model.parameters() if param.requires_grad], lr=lr_initial, weight_decay=5e-4
        )
...
def get_lr_scheduler(optimizer, scheduler_name, n_rounds=None):
    ...
    if scheduler_name == "sqrt":
        ...
        milestones = [
            n_rounds // 4, n_rounds // 2, 5 * (n_rounds // 8), 3 * (n_rounds // 4), 7 * (n_rounds // 8),
            15 * (n_rounds // 16)
        ]
        return optim.lr_scheduler.MultiStepLR(optimizer, milestones=milestones, gamma=0.5)
